...
public class SharedPreferencesCustom extends AppCompatActivity {
    ...
    public boolean loadIsContactChecked(Context context){
        SharedPreferences sharedPreferences = context.getSharedPreferences(SHARED_PREFS,MODE_PRIVATE);
        return sharedPreferences.getBoolean("isContactEnable",false);
    }

    public boolean loadDualSim(Context context){
        SharedPreferences sharedPreferences = context.getSharedPreferences(SHARED_PREFS,MODE_PRIVATE);
        return sharedPreferences.getBoolean("dualSim",true);
    }

    public String loadSim(Context context){
        SharedPreferences sharedPreferences = context.getSharedPreferences(SHARED_PREFS,MODE_PRIVATE);
        System.out.println(sharedPreferences.getString("sim","sim1"));
        return sharedPreferences.getString("sim","sim1");
    }
}
